...
axios.get(`https://restaurant-project-node-js.vercel.app/api/category`).then((apiResponce)=>{
...
function getAllMeals(){
  axios.get(`https://restaurant-project-node-js.vercel.app/api/menu`).then((responce)=>{
...
const filterMeal=allMeals.filter((meal)=>meal.productCategory.name==category)
  ...
  return (
    <>
     <section className="py-8" >
      <Header hightlight={'What We Serve'} 
       text={"our menu"} 
       decripOne={"We consider all the drivers of change giving you the components you need to"}
    ...
    ${
      ...
      >
       ...
       <div className=" grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 px-15 gap-4">
        {
          disolayMeals?disolayMeals.map((meal)=> <MealCard title={meal.productName}
           descrip={meal.productDescription} price={meal.productPrice} image={meal.imageUrl}  
           key={meal._id} ID={meal._id} category={meal.productCategory.name}></MealCard>)
         :<p>Meals Not Found</p>
        }
