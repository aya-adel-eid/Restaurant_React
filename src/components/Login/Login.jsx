...
export function Login() { 
 ...
 function Login(userData){
  ...
  setErrorMessage('');
 setLoading(true)
  axios.post(`https://restaurant-project-node-js.vercel.app/api/auth/login`,userData).then((resp)=>{
 setLoading(false)
  ...
  return (
     ...
     <div className="flex justify-center py-10 px-6 sm:px-1   bg-[#EEEEED] min-h-screen items-center">
      <div className="w-full lg:w-[50%] bg-white p-9 rounded-3xl">
        ...
        <div className="flex flex-col justify-center items-center py-1.5">
          ...
          <h3 className="py-1 font-semibold  text-mainText lg:font-bold text-lg lg:text-3xl">Welcome Back</h3>
          <span className="text-gray-500">Sign in to your Bistro Bliss account</span>
        </div>
    ...
    {/* password */}
            <div className="mb-4">
              ...
              />
              <span className="absolute top-1/2 -translate-y-1/2 right-3" onClick={()=>setShowPassword((prev)=>!prev)}>{showPassword?<i className="fa-solid fa-eye"></i>:<i className="fa-solid fa-eye-slash"></i>}</span>
          ...
          <div className="text-center">
            <p className="font-semibold">Don't have an account? <Link to={'/signUp'} className="text-main-500 font-bold">Create one</Link></p>
          </div>
      </div>
        </div>
    </>
